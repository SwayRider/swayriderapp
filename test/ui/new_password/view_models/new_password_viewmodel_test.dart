...
void main() {
  ...
  test(
    'execute calls AuthRepository.resetPassword with userId, token and newPassword',
    () async {
      when(
  ...
  test('re-entrant execute calls only invoke the repository once', () async {
    ...
    final first = viewModel.resetPassword.execute((
      'user-1',
      'token-1',
      'new-pw',
    ));
    final second = viewModel.resetPassword.execute((
      'user-1',
      'token-1',
      'new-pw',
    ));
